...
BOOL websocket_transport_write_wstream(rdpTransport* transport, wStream* sPacket,
                                       WEBSOCKET_OPCODE opcode)
{
	...
	status = transport_write(transport, sWS);
	Stream_Free(sWS, TRUE);
...
int websocket_transport_write(rdpTransport* transport, const BYTE* buf, int isize,
                              WEBSOCKET_OPCODE opcode)
{
	size_t payloadSize = 0;
	size_t fullLen = 0;
...
static int websocket_transport_read_data(rdpTransport* transport, BYTE* pBuffer, size_t size,
                                         websocket_context* encodingContext)
{
	int status = 0;
...
static int websocket_transport_read_discard(rdpTransport* transport,
                                            websocket_context* encodingContext)
{
	BYTE _dummy[256] = { 0 };
	int status = 0;
	...
	status = transport_read_bytes(transport, _dummy, sizeof(_dummy));
	if (status <= 0)
		return status;
...
static int websocket_transport_read_wstream(rdpTransport* transport, wStream* s,
                                            websocket_context* encodingContext)
{
	int status = 0;
	...
	status = transport_read_bytes(transport, Stream_Pointer(s), encodingContext->payloadLength);
	if (status <= 0)
		return status;
...
static BOOL websocket_transport_reply_close(rdpTransport* transport, wStream* s)
{
	/* write back close */
	wStream* closeFrame = NULL;
	uint16_t maskingKey1 = 0;
	...
	status = transport_write(transport, closeFrame);
	Stream_Free(closeFrame, TRUE);
...
static BOOL websocket_transport_reply_pong(rdpTransport* transport, wStream* s)
{
	wStream* closeFrame = NULL;
	uint32_t maskingKey = 0;
	int status = 0;
	...
	if (s != NULL)
		return websocket_transport_write_wstream(transport, s, WebsocketPongOpcode);
...
static int websocket_transport_handle_payload(rdpTransport* transport, BYTE* pBuffer, size_t size,
                                              websocket_context* encodingContext)
{
	int status = 0;
	...
	switch (effectiveOpcode)
	{
		case WebsocketBinaryOpcode:
		{
			status = websocket_transport_read_data(transport, pBuffer, size, encodingContext);
			if (status < 0)
				return status;
		...
		case WebsocketPingOpcode:
		{
			...
			status = websocket_transport_read_wstream(
			    transport, encodingContext->responseStreamBuffer, encodingContext);
			if (status < 0)
				return status;
			...
			{
				if (!encodingContext->closeSent)
					websocket_transport_reply_pong(transport,
					                               encodingContext->responseStreamBuffer);
		...
		case WebsocketCloseOpcode:
		{
			...
			if (encodingContext->payloadLength == 0)
			{
				websocket_transport_reply_close(transport, encodingContext->responseStreamBuffer);
				encodingContext->closeSent = TRUE;
		...
		default:
			...
			status = websocket_transport_read_discard(transport, encodingContext);
			if (status < 0)
				return status;
	}
	/* return how many bytes have been written to pBuffer.
...
int websocket_transport_read(rdpTransport* transport, BYTE* pBuffer, size_t size,
                             websocket_context* encodingContext)
{
	int status = 0;
	int effectiveDataLen = 0;
	...
	while (TRUE)
	{
		switch (encodingContext->state)
		{
			...
			{
				BYTE buffer[1];
				status = transport_read_bytes(transport, buffer, sizeof(buffer));
				if (status <= 0)
					return (effectiveDataLen > 0 ? effectiveDataLen : status);
			...
			{
				status =
				    websocket_transport_handle_payload(transport, pBuffer, size, encodingContext);
				if (status < 0)
					return (effectiveDataLen > 0 ? effectiveDataLen : status);
